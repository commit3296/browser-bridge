...
Move Chrome cookies between profiles using an encrypted local archive. Bookmarks and extension lists can be included.
...
It exports cookies into a password-protected archive file and imports that archive into another Chrome profile. All detected cookie domains are selected by default, the user must confirm that the encrypted file can keep websites signed in, and domains can be reviewed before export. Bookmarks and an extension list can be included with the same archive.
